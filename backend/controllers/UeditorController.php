...
class UeditorController extends \crazydb\ueditor\UEditorController
{
    ...
    protected function upload($fieldName, $config, $base64 = 'upload')
    {
        ...
        $info = $up->getFileInfo();
        if (($this->thumbnail or $this->zoom or $this->watermark) && 'SUCCESS' == $info['state'] && in_array($info['type'], ['.png', '.jpg', '.bmp', '.gif'], true)) {
    ...
    protected function getFiles($path, $allowFiles, &$files = [])
    {
        ...
        if (in_array(basename($path), $this->ignoreDir, true)) {
            return;
        }
