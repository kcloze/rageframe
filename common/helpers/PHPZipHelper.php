...
class PHPZipHelper
{
    ...
    public function unZip($zipfile, $to, $index = [-1])
    {
        ...
        for ($i = 0; $i < $cdir['entries']; ++$i) {
            ...
            if (in_array('-1', $index, true) || in_array($i, $index, true)) {
    ...
    public function GetZipInnerFilesInfo($zipfile)
    {
        ...
        for ($i = 0; $i < $centd['entries']; ++$i) {
            ...
            $info            = [
                ...
                'folder'          => (0x41FF0010 == $header['external'] || 16 == $header['external']) ? 1 : 0,  // 是否为文件夹
    ...
    private function ReadCentralDir($zip, $zipfile)
    {
        ...
        $centd['comment']      = (0 != $data['comment_size']) ? fread($zip, $data['comment_size']) : '';  // 注释
    ...
    private function ReadCentralFileHeaders($zip)
    {
        ...
        $header['filename'] = (0 != $header['filename_len']) ? fread($zip, $header['filename_len']) : '';
        $header['extra']    = (0 != $header['extra_len']) ? fread($zip, $header['extra_len']) : '';
        $header['comment']  = (0 != $header['comment_len']) ? fread($zip, $header['comment_len']) : '';
    ...
    private function ReadFileHeader($zip)
    {
        ...
        $header['extra']           = (0 != $data['extra_len']) ? fread($zip, $data['extra_len']) : '';
